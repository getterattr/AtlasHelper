...
5. Check `.claude/docs/internal/adr/` for architectural decisions before proposing changes to scope or persistence:
...
6. Load relevant skills based on your task (skills live in `~/.claude/skills/` and load on demand):
...
7. Plugins are compiled by `Loader.exe` on HUD startup. Do not run `dotnet build` to produce runtime DLLs.
